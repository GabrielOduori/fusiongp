...
Methodology:
- When USE_EPA_IN_TRAINING=False: Train on non-EPA sources (if enabled), evaluate on held-out EPA
- When USE_EPA_IN_TRAINING=True: Train on EPA (optionally with covariates), evaluate on EPA holdout
...
# Use split input files (not a pre-merged FusionData.csv) for flexibility.
DATA_DIR = Path("/media/gabriel-oduori/SERVER/dev_space/FusionGP/data_test")
USE_SPLIT_DATA = True
DATA_PATH = DATA_DIR / "FusionData.csv"  # Fallback only; split data is the default.

HAS_SATELLITE = True
USE_SATELLITE_AS_COVARIATE = True  # Align with st_gp_fusion: satellite as covariate, not observation
HAS_SATELLITE_OBS = HAS_SATELLITE and not USE_SATELLITE_AS_COVARIATE
COVARIATE_COLUMNS = ["satellite_no2"] if USE_SATELLITE_AS_COVARIATE else []
COVARIATE_DISTANCE_COLUMNS = []
AGGREGATE_TO_DAILY = True  # Aggregate hourly data to daily means before training
...
MODEL_CONFIG = {
    ...
    "initial_noise": {"epa": 0.3} if not HAS_SATELLITE_OBS else {"epa": 0.3, "satellite": 2.0},
}

TRAINING_CONFIG = {
    "learning_rate": 0.01, #0.01
    "n_epochs": 50,  # Reduced for faster iteration while we align preprocessing with st_gp_fusion
    "batch_size": 2048,
    "val_interval": 5,
...
USE_EPA_IN_TRAINING = True  # Train on EPA to align with st_gp_fusion setup
...
AVAILABLE_SOURCES = ["epa"] + (["satellite"] if HAS_SATELLITE_OBS else [])
...
# Prior mean comes from LUR predictions (predicted_no2) in the split data.
# Optionally inject traffic volume anomalies into the prior mean.
USE_TRAFFIC_IN_PRIOR = True
TRAFFIC_WEIGHT = 1.0  # Weight for traffic anomaly term in prior mean
GRID_PRIOR_COLUMN = "predicted_no2_traffic" if USE_TRAFFIC_IN_PRIOR else "predicted_no2"
...
def _load_time_map(data_dir: Path) -> dict:
    """Return inverse time map: index -> timestamp string."""
    import json
    mapping_path = data_dir / "id_mappings.json"
    with mapping_path.open("r") as fh:
        mappings = json.load(fh)
    time_map = mappings.get("time_map", {})
    # Invert to map int index -> timestamp string.
    return {int(v): k for k, v in time_map.items()}


def _load_grid_id_map(data_dir: Path) -> dict:
    """Return grid_id map: grid_id string -> int."""
    import json
    mapping_path = data_dir / "id_mappings.json"
    with mapping_path.open("r") as fh:
        mappings = json.load(fh)
    return mappings.get("grid_id_map", {})


def build_fusion_dataframe_from_parts(data_dir: Path) -> pd.DataFrame:
    """
    Build a unified dataframe from split files in data_dir.

    We intentionally exclude synthetic/low-cost fields. Coordinates are taken
    from grid_coordinates.csv (x/y) and mapped into the DataLoader's
    latitude/longitude slots for consistency with the grid system.
    """
    grid_df = pd.read_csv(data_dir / "grid_coordinates.csv")
    grid_df = grid_df.rename(columns={"x": "latitude", "y": "longitude"})

    # Prior mean from LUR predictions (grid_id like "0_0"); map to integer grid_id.
    lur_df = pd.read_csv(data_dir / "lur_predictions.csv", usecols=["grid_id", "predicted_no2"])
    grid_id_map = _load_grid_id_map(data_dir)
    lur_df["grid_id"] = lur_df["grid_id"].map(grid_id_map)
    lur_df = lur_df.dropna(subset=["grid_id"]).copy()
    lur_df["grid_id"] = lur_df["grid_id"].astype(int)

    # Satellite retrievals (grid_id/time are integer indices).
    sat_df = pd.read_csv(data_dir / "satellite_retrievals.csv", usecols=["grid_id", "time", "value"])
    sat_df = sat_df.rename(columns={"value": "satellite_no2"})

    # EPA observations at station locations; we snap to nearest grid for consistency.
    epa_df = pd.read_csv(data_dir / "epa_observations.csv", usecols=["x", "y", "time", "value"])
    epa_df = epa_df.rename(columns={"value": "epa_no2"})

    # Map time indices to real timestamps using id_mappings.json.
    time_map_inv = _load_time_map(data_dir)
    sat_df["timestamp"] = sat_df["time"].map(time_map_inv)
    epa_df["timestamp"] = epa_df["time"].map(time_map_inv)

    # Drop rows with unmapped timestamps to avoid NaN timestamps downstream.
    sat_df = sat_df.dropna(subset=["timestamp"]).copy()
    epa_df = epa_df.dropna(subset=["timestamp"]).copy()

    # Parse timestamps and optionally aggregate to daily means.
    sat_df["timestamp"] = pd.to_datetime(sat_df["timestamp"], errors="coerce")
    epa_df["timestamp"] = pd.to_datetime(epa_df["timestamp"], errors="coerce")
    sat_df = sat_df.dropna(subset=["timestamp"]).copy()
    epa_df = epa_df.dropna(subset=["timestamp"]).copy()

    if AGGREGATE_TO_DAILY:
        sat_df["timestamp"] = sat_df["timestamp"].dt.floor("D")
        epa_df["timestamp"] = epa_df["timestamp"].dt.floor("D")

    # Build KD-tree for nearest grid assignment of EPA points.
    grid_xy = grid_df[["latitude", "longitude"]].values
    tree = cKDTree(grid_xy)
    epa_xy = epa_df[["x", "y"]].values
    distances, nearest_idx = tree.query(epa_xy, k=1)
    # Only snap to grid if within 500 meters to avoid misassignment.
    max_snap_m = 500.0
    epa_df["grid_id"] = grid_df.iloc[nearest_idx]["grid_id"].values
    epa_df.loc[distances > max_snap_m, "grid_id"] = np.nan
    epa_df = epa_df.dropna(subset=["grid_id"]).copy()
    epa_df["grid_id"] = epa_df["grid_id"].astype(int)

    # Aggregate satellite and EPA to grid_id/timestamp.
    sat_base = (
        sat_df.groupby(["grid_id", "timestamp"], as_index=False)[["satellite_no2"]]
        .mean()
    )
    epa_base = (
        epa_df.groupby(["grid_id", "timestamp"], as_index=False)[["epa_no2"]]
        .mean()
    )

    # Union of grid_id/timestamp pairs across sources.
    base = pd.merge(sat_base, epa_base, on=["grid_id", "timestamp"], how="outer")

    # Attach grid coordinates.
    base = base.merge(grid_df[["grid_id", "latitude", "longitude"]], on="grid_id", how="left")

    # Attach prior mean per grid (LUR).
    base = base.merge(lur_df, on="grid_id", how="left")

    # Optionally inject traffic volume anomalies into the prior mean.
    if USE_TRAFFIC_IN_PRIOR:
        traffic_df = pd.read_csv(
            data_dir / "traffic_timeseries.csv",
            usecols=["grid_id", "traffic_end_time", "traffic_volume"],
        )
        # Map grid_id string -> int for alignment.
        ...
        # Normalize timestamps to the same day-based format.
        traffic_df["traffic_end_time"] = pd.to_datetime(traffic_df["traffic_end_time"], errors="coerce")
        traffic_df = traffic_df.dropna(subset=["traffic_end_time"]).copy()
        if AGGREGATE_TO_DAILY:
            traffic_df["timestamp"] = traffic_df["traffic_end_time"].dt.floor("D")
        else:
            traffic_df["timestamp"] = traffic_df["traffic_end_time"].dt.floor("h")

        # Aggregate traffic volume per grid/time and log-transform.
        traffic_agg = (
            traffic_df.groupby(["grid_id", "timestamp"], as_index=False)[["traffic_volume"]]
            .mean()
        )
        traffic_agg["traffic_volume"] = np.log1p(traffic_agg["traffic_volume"])

        # Z-score per grid to capture temporal anomalies (st_gp_fusion behavior).
        traffic_agg["traffic_z"] = traffic_agg.groupby("grid_id")["traffic_volume"].transform(
            lambda s: (s - s.mean()) / (s.std() if s.std() else 1.0)
        )
        traffic_agg = traffic_agg.drop(columns=["traffic_volume"])

        base = base.merge(traffic_agg, on=["grid_id", "timestamp"], how="left")
        base["traffic_z"] = base["traffic_z"].fillna(0.0)
        base["predicted_no2_traffic"] = base["predicted_no2"] + TRAFFIC_WEIGHT * base["traffic_z"]
    else:
        base["predicted_no2_traffic"] = base["predicted_no2"]

    if AGGREGATE_TO_DAILY:
        # Ensure any residual duplicates are collapsed to daily means.
        base = (
            base.groupby(["grid_id", "timestamp"], as_index=False)
            .mean(numeric_only=True)
        )
        base["timestamp"] = base["timestamp"].dt.strftime("%Y-%m-%d")

    # Ensure expected column order.
    return base[["grid_id", "latitude", "longitude", "timestamp", "epa_no2", "satellite_no2", GRID_PRIOR_COLUMN]]


def build_point_analysis_grid_from_coordinates(data_dir: Path) -> AnalysisGrid:
    """
    Build a point-based AnalysisGrid from grid_coordinates.csv.

    This avoids creating a full Cartesian mesh (which is enormous for irregular grids)
    while still supporting prediction via AnalysisGrid APIs.
    """
    from src.data.grid import AnalysisGrid, GridInfo

    grid_df = pd.read_csv(data_dir / "grid_coordinates.csv")
    lats = grid_df["x"].to_numpy()
    lons = grid_df["y"].to_numpy()

    # Create a minimal AnalysisGrid instance with a single-column grid.
    grid = AnalysisGrid.__new__(AnalysisGrid)
    grid.name = "FusionGP_PointGrid"
    grid.info = GridInfo(
        ...
        lon_res=0.0,
        lat_res=0.0,
        lons=np.array([float(np.min(lons))]),
        lats=lats,
    )
    # Store points as a single-column grid (n_points x 1).
    grid.lon_grid = lons.reshape(-1, 1)
    grid.lat_grid = lats.reshape(-1, 1)
...
def main():
    ...
    # EPA-in-training is allowed here to align with the st_gp_fusion setup.
    ...
    # Build a unified dataframe from split files when USE_SPLIT_DATA is enabled.
    if USE_SPLIT_DATA:
        if not DATA_DIR.exists():
            raise FileNotFoundError(
                f"Data directory not found at {DATA_DIR}. "
                "Please ensure split files exist."
            )
        df_outlier = build_fusion_dataframe_from_parts(DATA_DIR)
        data_path_for_run = Path("/tmp/fusiongp_split_data.csv")
    else:
    ...
    if EPA_OUTLIER_FILTER:
        if "epa_no2" in df_outlier.columns:
            ...
            if n_outliers > 0:
                ...
                if USE_SPLIT_DATA:
                    cleaned_path = Path("/tmp/fusiongp_split_data_epa_cleaned.csv")
                else:
        ...
        # Persist the split dataset to a temp CSV for the existing DataLoader path.
        if USE_SPLIT_DATA:
            df_outlier.to_csv(data_path_for_run, index=False)
    if USE_SPLIT_DATA and not data_path_for_run.exists():
        # Ensure we have a CSV on disk for DataLoader even if no outliers were found.
        df_outlier.to_csv(data_path_for_run, index=False)
    ...
    header_cols = df_outlier.columns if USE_SPLIT_DATA else pd.read_csv(data_path_for_run, nrows=0).columns
    ...
    if HAS_SATELLITE_OBS or USE_SATELLITE_AS_COVARIATE:
    ...
    loader = DataLoader(
        ...
        column_mapping={
            ...
            "longitude": "longitude",
            "timestamp": "timestamp",
            **({"satellite": "satellite_no2"} if HAS_SATELLITE_OBS else {}),
    ...
    if PRE_CALIBRATE_SOURCES and not USE_EPA_IN_TRAINING and HAS_SATELLITE_OBS:
    ...
    # Normalize targets to align with st_gp_fusion preprocessing (EPA-based z-score).
    ...
    if RUN_CASE0_EPA_ONLY:
        ...
        if not USE_EPA_IN_TRAINING:
            training_sources = ["satellite"] if HAS_SATELLITE_OBS else []
            train_data = filter_data_by_sources(train_data, training_sources)
            val_data = filter_data_by_sources(val_data, training_sources)
            model_config = MODEL_CONFIG.copy()
            model_config["sources"] = training_sources
            globals()["USE_MULTITASK_MODEL"] = False
            globals()["PRIMARY_SOURCE"] = "satellite" if HAS_SATELLITE_OBS else "epa"
        else:
            model_config = MODEL_CONFIG.copy()
    ...
    if len(timestamps) > 2:
        # Limit grid prediction to 2 timestamps for faster iteration.
        timestamps = np.array([timestamps[0], timestamps[-1]])
    ...
    if USE_ANALYSIS_GRID:
        if USE_PRIOR_GRID_FOR_PREDICTION and USE_GRID_PRIOR:
            ...
            if USE_SPLIT_DATA:
                # Use a point-based grid (irregular) from grid_coordinates.csv.
                # This avoids an oversized Cartesian mesh.
                analysis_grid = build_point_analysis_grid_from_coordinates(DATA_DIR)
            else:
    ...
    # Compute metrics on raw predictions (as-is) and bias-corrected predictions.
    # WHY: Bias is dominating R²; reporting a bias-corrected view mirrors st_gp_fusion.
    epa_metrics_raw = evaluator.evaluate(
    ...
    epa_metrics_bias = evaluator.evaluate(
        y_true=epa_test_orig,
        y_pred_mean=pred_mean_bias_corrected,
        y_pred_std=pred_std_at_epa,
    )
    ...
    # Use bias-corrected metrics for primary reporting.
    epa_metrics = epa_metrics_bias

    print("\n" + "="*50)
    print("Evaluation Results (Fusion vs EPA Ground Truth, Bias-Corrected)")
    print("="*50)
