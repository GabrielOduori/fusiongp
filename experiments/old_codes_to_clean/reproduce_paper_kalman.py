...
Methodology:
- When USE_EPA_IN_TRAINING=False: Train on LCS+Satellite, evaluate on held-out EPA
...
COVARIATE_COLUMNS = ["satellite_no2", "model_no2_10m", "traffic_volume"]
...
# Kalman Filter (post-processing) configuration
KF_ENABLED = True
KF_MEASUREMENT_SOURCE = "satellite"  # Use satellite for temporal updates (no synthetic/low_cost data)
KF_PROCESS_NOISE = 0.5
KF_MEASUREMENT_NOISE = 2.0


def build_multi_case_definitions():
    """Return case definitions for multi-case evaluation (no synthetic/low-cost data)."""
    return [
        ...
        {"name": "Case 3 (EPA + Satellite)", "sources": ["epa", "satellite"], "mode": "train", "use_prior": True},
    ]
...
USE_EPA_IN_TRAINING = False  # EPA is evaluation-only for this experiment
...
HAS_SATELLITE = True  # Use real TROPOMI satellite data
RUN_CASE0_EPA_ONLY = False

AVAILABLE_SOURCES = ["epa", "satellite"]  # No synthetic/low_cost data
...
def calibrate_sources_to_epa(data, sources_to_calibrate=('low_cost', 'satellite')):
    """
    ...
    sources_to_calibrate : tuple of str
        Sources to calibrate (default: low_cost, satellite).
...
class KalmanFilter:
    """
    Simple 1D Kalman Filter for temporal updating of NO₂ estimates.
    State: x (NO₂ concentration)
...
def apply_kalman_filter_temporal(
    df,
    gp_predictions,
    gp_variances,
    measurement_column,
    ...
    """
    Apply KF updates over time per location.
    Uses GP predictions as initial state, then updates with measurements
    when available.
    """
    n_points = len(gp_predictions)
    updated_predictions = np.zeros(n_points)
    updated_variances = np.zeros(n_points)

    df_copy = df.copy()
    df_copy["gp_pred"] = gp_predictions
    df_copy["gp_var"] = gp_variances

    if "timestamp" in df_copy.columns:
        df_copy = df_copy.sort_values("timestamp")

    if "grid_id" in df_copy.columns and df_copy["grid_id"].notna().any():
        group_keys = ["grid_id"]
    else:
        group_keys = ["latitude", "longitude"]

    for _, group in df_copy.groupby(group_keys):
        first = group.iloc[0]
        kf = KalmanFilter(
            initial_state=first["gp_pred"],
            initial_variance=first["gp_var"],
        ...
        for i, (idx, row) in enumerate(group.iterrows()):
            if i == 0:
                updated_predictions[idx] = kf.x
                updated_variances[idx] = kf.P
                continue

            measurement = row[measurement_column]
            if np.isnan(measurement):
                state, var = kf.predict()
            else:
                state, var = kf.filter_step(measurement)

            updated_predictions[idx] = state
            updated_variances[idx] = var

    return updated_predictions, updated_variances
...
def save_experiment_summary(experiment_dir, model, trainer, learned_params,
    ...
    with open(summary_file, 'w') as f:
        ...
        if USE_GRID_PRIOR:
            ...
            if USE_TROPOMI_PATTERN:
                f.write(
                    f"Prior augmentation: {AUGMENTED_PRIOR_COLUMN} "
                    f"(TROPOMI pattern, {TROPOMI_CALIBRATION_DAYS} days, "
        ...
        if KF_ENABLED:
            f.write(
                f"Kalman Filter: enabled (measurement={KF_MEASUREMENT_SOURCE}, "
                f"Q={KF_PROCESS_NOISE}, R={KF_MEASUREMENT_NOISE})\n"
            )
        ...
        if kf_metrics is not None:
            f.write(f"Evaluation Metrics (KF vs EPA ground truth):\n")
            f.write(f"{'-'*70}\n")
            for key, val in kf_metrics.to_dict().items():
                if isinstance(val, (int, float, np.number)):
                    f.write(f"{key}: {val:.4f}\n")
                else:
                    f.write(f"{key}: {val}\n")
...
def main():
    ...
    # -------------------------------------------------------------------------
    # Aggregate hourly data to daily means to match satellite temporal resolution.
    # This reduces noise and aligns EPA/satellite observations to the same time grid.
    # -------------------------------------------------------------------------
    print("\n   Aggregating hourly data to daily means...")
    df_raw = pd.read_csv(data_path_for_run)

    # Drop synthetic/low-cost columns if present
    synthetic_cols = [c for c in ["synthetic_no2", "base_syhthetic_no2"] if c in df_raw.columns]
    if synthetic_cols:
        df_raw = df_raw.drop(columns=synthetic_cols)
        print(f"   ✓ Dropped synthetic columns: {synthetic_cols}")

    # Normalize grid_id to integer indices for consistent merges.
    if "grid_id" in df_raw.columns:
        grid_id_series = df_raw["grid_id"]
        if grid_id_series.dtype == object:
            grid_id_numeric = pd.to_numeric(grid_id_series, errors="coerce")
            if grid_id_numeric.notna().any() and grid_id_numeric.isna().sum() == 0:
                df_raw["grid_id"] = grid_id_numeric.astype(int)
            elif ID_MAP_PATH.exists():
                with ID_MAP_PATH.open("r") as fh:
                    id_maps = json.load(fh)
                grid_id_map = id_maps.get("grid_id_map", {})
                df_raw["grid_id"] = grid_id_series.astype(str).map(grid_id_map)
                before = len(df_raw)
    ...
    # Columns to aggregate with mean (numeric observation + model columns)
    obs_cols = ["epa_no2", "satellite_no2", "model_no2_10m", "model_no_100m", "epa_interpolated"]
    obs_cols = [c for c in obs_cols if c in df_raw.columns]

    # Group by grid_id + date, take daily mean of observations
    agg_dict = {c: "mean" for c in obs_cols}
    # Keep first value for static columns
    ...
    if TRAFFIC_PATH.exists() and ID_MAP_PATH.exists():
        try:
            ...
            traffic_df["date"] = traffic_df["traffic_end_time"].dt.date
    ...
    # Convert date back to timestamp string (noon of each day)
    df_daily["timestamp"] = pd.to_datetime(df_daily["date"]).astype(str)
    df_daily = df_daily.drop(columns=["date"])

    n_hourly = len(df_raw)
    n_daily = len(df_daily)
    n_days = df_daily["timestamp"].nunique()
    print(f"   ✓ Aggregated: {n_hourly:,} hourly rows → {n_daily:,} daily rows ({n_days} days)")
    print(f"     EPA daily obs: {df_daily['epa_no2'].notna().sum():,}")
    print(f"     Satellite daily obs: {df_daily['satellite_no2'].notna().sum():,}")

    # Save daily CSV for DataLoader
    daily_path = DATA_PATH.with_name(f"{DATA_PATH.stem}_daily.csv")
    ...
    loader = DataLoader(
        ...
        column_mapping={
            ...
            # NOTE: synthetic/low_cost columns intentionally excluded
        },
        covariate_columns=COVARIATE_COLUMNS,
    )
    data = loader.load()
    # Keep satellite observations only at their timestamps (no imputation).
    if "satellite" in data.source_masks:
        sat_mask = data.source_masks["satellite"]
        sat_count = int(sat_mask.sum())
        sat_times = np.unique(data.timestamps[sat_mask]) if sat_count > 0 else []
        print(
            f"   ✓ Satellite observations: {sat_count:,} points across {len(sat_times)} timestamps "
            f"(sparse, no imputation)"
        )
    ...
    if PRE_CALIBRATE_SOURCES and not USE_EPA_IN_TRAINING:
        print("\n   Pre-calibrating satellite to EPA scale...")
        sources_to_calibrate = ['satellite'] if HAS_SATELLITE else []
        if sources_to_calibrate:
            data, calibration_params = calibrate_sources_to_epa(
                data, sources_to_calibrate=tuple(sources_to_calibrate)
            )
    ...
    if RUN_CASE0_EPA_ONLY:
        ...
        if not USE_EPA_IN_TRAINING:
            ...
            model_config["sources"] = training_sources
            globals()["USE_MULTITASK_MODEL"] = False
            globals()["PRIMARY_SOURCE"] = "satellite"
        else:
            model_config = MODEL_CONFIG.copy()
    ...
    if USE_GRID_PRIOR:
        ...
        if USE_TROPOMI_PATTERN:
            print(f"   → Building TROPOMI pattern prior ({TROPOMI_CALIBRATION_DAYS} days, "
                  f"{'restricted' if TROPOMI_RESTRICTED else 'full'})...")
            df_prior = build_tropomi_pattern_prior(
                df_prior,
            ...
            prior_column = AUGMENTED_PRIOR_COLUMN
        else:
    ...
    kf_predictions = None
    if KF_ENABLED:
        if KF_MEASUREMENT_SOURCE not in test_data.observations:
            print(f"   ⚠ KF disabled: measurement source '{KF_MEASUREMENT_SOURCE}' not found.")
        else:
            print(f"   Applying Kalman Filter using '{KF_MEASUREMENT_SOURCE}' measurements...")
            meas = test_data.observations[KF_MEASUREMENT_SOURCE]
            if scalers.normalize_targets and KF_MEASUREMENT_SOURCE in scalers.target_std:
                meas = meas * scalers.target_std[KF_MEASUREMENT_SOURCE] + scalers.target_mean[KF_MEASUREMENT_SOURCE]
            ...
            df_kf = pd.DataFrame({
                ...
                KF_MEASUREMENT_SOURCE: meas,
            })

            kf_mean, kf_var = apply_kalman_filter_temporal(
                df_kf,
                gp_predictions=predictions.mean,
                gp_variances=predictions.std ** 2,
                measurement_column=KF_MEASUREMENT_SOURCE,
            ...
            print(f"   ✓ KF mean/std: {np.nanmean(kf_mean):.4f}, {np.nanmean(kf_std):.4f}")
    ...
    if kf_predictions is not None:
        kf_pred_mean_at_epa = kf_predictions["mean"][epa_eval_mask]
        kf_pred_std_at_epa = kf_predictions["std"][epa_eval_mask]
        kf_metrics = evaluator.evaluate(
            y_true=epa_test_orig,
            y_pred_mean=kf_pred_mean_at_epa,
            y_pred_std=kf_pred_std_at_epa,
        )
        print("\n" + "="*50)
        print("Evaluation Results (KF vs EPA Ground Truth)")
        print("="*50)
        print(kf_metrics.summary())
    ...
    print(f"      Columns: grid_id, latitude, longitude, timestamp, predicted_mean, predicted_std, ci_lower_95, ci_upper_95, epa_no2, low_cost_no2, satellite_no2")
