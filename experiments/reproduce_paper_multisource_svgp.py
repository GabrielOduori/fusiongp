"""
Multi-source SVGP (EPA held-out) with model+TROPOMI prior.
...
DATA_PATH = Path("/media/gabriel-oduori/SERVER/dev_space/data-tools/airquality/airqualitybase+simulated+epa+tropomi_updated_2.csv")
...
MODEL_CONFIG = {
    ...
    "initial_noise": {"epa": 0.3, "low_cost": 3.0, "satellite": 2.0},
}
...
KF_ENABLED = False
KF_MEASUREMENT_SOURCE = "low_cost"  # unused when KF_DISABLED
...
def build_multi_case_definitions():
    ...
    return [
        ...
        {"name": "Case 2 (Prior + LCS)", "sources": ["low_cost"], "mode": "train", "use_prior": True},
        {"name": "Case 3 (Prior + Satellite)", "sources": ["satellite"], "mode": "train", "use_prior": True},
...
AVAILABLE_SOURCES = ["epa", "low_cost"] + (["satellite"] if HAS_SATELLITE else [])
...
GRID_PRIOR_COLUMN = "model_no_100m"  # Column containing prior values (if using new data format)
...
AUGMENTED_PRIOR_COLUMN = "model_no_100m_tropomi"
...
def main():
    ...
    if PRE_CALIBRATE_SOURCES and not USE_EPA_IN_TRAINING:
        if HAS_SATELLITE:
            print("\n   Pre-calibrating LCS and satellite to EPA scale...")
        else:
            print("\n   Pre-calibrating LCS to EPA scale...")
        sources_to_calibrate = ['low_cost']
        if HAS_SATELLITE:
            sources_to_calibrate.append('satellite')
    ...
    if RUN_CASE0_EPA_ONLY:
        ...
        if not USE_EPA_IN_TRAINING:
            training_sources = ["low_cost"]
            if HAS_SATELLITE:
                training_sources.append("satellite")
            train_data = filter_data_by_sources(train_data, training_sources)
            val_data = filter_data_by_sources(val_data, training_sources)
            model_config = MODEL_CONFIG.copy()
            model_config["sources"] = training_sources
            globals()["USE_MULTITASK_MODEL"] = False
            globals()["PRIMARY_SOURCE"] = "low_cost"
        else:
            model_config = MODEL_CONFIG.copy()
    ...
    for timestamp in timestamps_for_export:
        ...
        low_cost_col = next(
            (c for c in ["synthetic_no2", "low_cost_data", "low_cost_no2", "low_cost"] if c in df_original.columns),
            None,
        )
        satellite_col = next(
        ...
        if low_cost_col:
            obs_cols.append(low_cost_col)
            rename_map[low_cost_col] = "low_cost_no2"
        if satellite_col:
        ...
        if "low_cost_no2" not in timestamp_df.columns:
            timestamp_df["low_cost_no2"] = np.nan
